...
| --- | --- | --- | --- | --- |
| MD-02 | model-level | Validity | Believability + Representational consistency | free (compile-time) |
...
The default `NUMERIC` on BQ is `(38, 9)`, but explicit precision protects against accidental scale drift. See [`../entity/EN-06-type-stable-join.md`](../entity/EN-06-type-stable-join.md).
...
This selector fires when any contract column's `data_type` changes vs production. Couple with `WARN_ERROR_OPTIONS` to make it a hard build failure. See [`MD-03-versioning-cutover.md`](./MD-03-versioning-cutover.md) for how to handle the failure (bump a version).
...
| **+ Versioning** | Ready for breaking-change governance. See [`MD-03-versioning-cutover.md`](./MD-03-versioning-cutover.md). |
...
- [`MD-03-versioning-cutover.md`](./MD-03-versioning-cutover.md) — how to evolve a contract safely
- [`../entity/EN-06-type-stable-join.md`](../entity/EN-06-type-stable-join.md) — the cross-model type-stability case
- [`../time/TM-SC-03-timezone-contract.md`](../time/TM-SC-03-timezone-contract.md) — TIMESTAMP vs DATETIME contract
