...
| --- | --- | --- | --- | --- |
| MD-10 | model-level | Validity | Believability + Interpretability | cheap |
...
Keep the `data_type: json` (or `string`) contract — it catches the column being dropped or retyped wholesale at parse time. `json_schema` catches the subtler "type held, structure moved" case at runtime. Defence in depth, same as [`MD-02-contracts.md`](./MD-02-contracts.md) describes for the rest of the model.
...
|--------|----------------|--------------|
| `contract` `data_type: json` | dbt core | **Always** — pins the *cell* type at parse time. Necessary but not sufficient. See [`MD-02-contracts.md`](./MD-02-contracts.md). |
...
- [`MD-02-contracts.md`](./MD-02-contracts.md) — the cell-level type contract this sits inside (MD-02)
- [`MD-08-schema-changes.md`](./MD-08-schema-changes.md) — drift of the *relation's* columns rather than within one (MD-08)
- [`../dimension/DM-01-accepted-values.md`](../dimension/DM-01-accepted-values.md) — enum validation once a JSON key is extracted to a column (DM-01)
