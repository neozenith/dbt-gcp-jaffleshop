...
| --- | --- | --- | --- | --- |
| MD-07 | model-level | Accuracy + Timeliness | Accuracy + Timeliness | history-bound |

A fixed row-count band ([`MD-06-row-count-band.md`](./MD-06-row-count-band.md)) catches catastrophes; a learned-band anomaly detector catches the more subtle "today's count is 40% below the seasonal trend". Elementary maintains the metrics history incrementally, so subsequent runs only scan the detection window.
...
`volume_anomalies` doesn't detect "the table is fresh but always has the same data". Pair with [`../time/TM-AU-01-freshness-source-and-model.md`](../time/TM-AU-01-freshness-source-and-model.md):
...
| `elementary.dimension_anomalies` | elementary | When the table is OK globally but a slice may go dark. See [`../dimension/DM-05-dimension-anomalies.md`](../dimension/DM-05-dimension-anomalies.md). |
...
- **Less than ~2× `training_period` of history** (≥ 56 days for `training_period: 28d`). Use fixed-band [`MD-06-row-count-band.md`](./MD-06-row-count-band.md) until history accumulates.
...
- [`MD-06-row-count-band.md`](./MD-06-row-count-band.md) — fixed-band complement / fallback
- [`../dimension/DM-05-dimension-anomalies.md`](../dimension/DM-05-dimension-anomalies.md) — per-slice version
- [`../measure/MS-05-distribution-anomaly.md`](../measure/MS-05-distribution-anomaly.md) — same engine on measure columns
