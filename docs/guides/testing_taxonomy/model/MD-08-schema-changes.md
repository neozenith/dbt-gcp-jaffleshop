...
| --- | --- | --- | --- | --- |
| MD-08 | model-level | Validity | Believability | history-bound |

A dbt `contract` ([`MD-02-contracts.md`](./MD-02-contracts.md)) pins the shape of a model **you build** — it fails at parse time if one of *your* columns drifts. It cannot see a column that an **upstream source you don't own** added, dropped, or retyped, because that change happens outside your DAG's compiled SQL. Elementary's schema-change tests watch a source (or model) relation's actual column set and types over time and fire when they move.
...
Reuse the Elementary setup from [`MD-07-volume-anomaly.md`](./MD-07-volume-anomaly.md). These tests live on **sources**, which is where un-owned drift enters the project.
...
flowchart LR
    ...
    classDef warn fill:#b453092e,stroke:#f59e0b,stroke-width:2px
```
...
|--------|----------------|--------------|
| `contract.enforced: true` | dbt core | For models **you build** — parse-time, free, no history. Always prefer where it applies. See [`MD-02-contracts.md`](./MD-02-contracts.md). |
...
- [`MD-02-contracts.md`](./MD-02-contracts.md) — the parse-time contract for models you own (MD-02)
- [`MD-10-json-schema.md`](./MD-10-json-schema.md) — drift *inside* a single semi-structured column (MD-10)
- [`../entity/EN-06-type-stable-join.md`](../entity/EN-06-type-stable-join.md) — type drift on a specific join key
