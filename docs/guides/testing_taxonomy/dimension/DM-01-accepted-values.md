...
| --- | --- | --- | --- | --- |
| DM-01 | dimension | Validity | Believability + Interpretability | cheap |
...
See [`DM-02-cardinality-guard.md`](./DM-02-cardinality-guard.md) for bounded ranges (`>= 4 AND <= 6`).
...
If three different models reference the same `region` set, put the canonical values in `seeds/regions.csv` and use `relationships` against the seed. See [`DM-03-conformed-dimension.md`](./DM-03-conformed-dimension.md).
...
| `relationships` against a seed | dbt core | When the value list is large (~20+) or shared across many models. See [`DM-03-conformed-dimension.md`](./DM-03-conformed-dimension.md). |
...
- [`DM-02-cardinality-guard.md`](./DM-02-cardinality-guard.md) — the count-based alternative
- [`DM-03-conformed-dimension.md`](./DM-03-conformed-dimension.md) — seed-driven enum for shared dimensions
- [`DM-04-mutual-exclusivity.md`](./DM-04-mutual-exclusivity.md) — when two sibling flags must partition the universe
