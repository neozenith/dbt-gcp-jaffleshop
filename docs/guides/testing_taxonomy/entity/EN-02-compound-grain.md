...
| --- | --- | --- | --- | --- |
| EN-02 | entity | Uniqueness | Concise representation | scan-bound |
...
| Concatenated `unique` on `MD5(a \|\| b)` | dbt core | **Anti-pattern.** Hash collisions are real (see [`EN-05-surrogate-collision-guard.md`](./EN-05-surrogate-collision-guard.md)). Use the dbt-utils test. |
...
- **Models with a single-column grain.** Use `unique` ([`EN-01-unique-key.md`](./EN-01-unique-key.md)) — composite tests on a single column are over-engineering.
...
- [`EN-01-unique-key.md`](./EN-01-unique-key.md) — single-column variant
- [`../model/MD-01-grain-test.md`](../model/MD-01-grain-test.md) — the model-level "every model has a grain" rule
- [`../time/TM-AU-02-scd2-quartet.md`](../time/TM-AU-02-scd2-quartet.md) — the four-test combination for SCD2 dims, which includes a compound-grain test
