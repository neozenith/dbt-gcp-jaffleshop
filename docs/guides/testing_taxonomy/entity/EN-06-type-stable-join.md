...
| --- | --- | --- | --- | --- |
| EN-06 | entity | Validity | Believability + Representational consistency | free (compile-time) |
...
The contract preflight compares precision; otherwise type-alias normalisation may silently coerce `numeric` and `numeric(38, 2)` as equivalent (see [`../model/MD-02-contracts.md`](../model/MD-02-contracts.md) §5.3).
...
- **Models with `ephemeral` or `materialized_view` materializations** — contracts aren't supported on these (see [`../model/MD-02-contracts.md`](../model/MD-02-contracts.md) §4).

## See also

- [`../model/MD-02-contracts.md`](../model/MD-02-contracts.md) — the full contract reference
- [`EN-01-unique-key.md`](./EN-01-unique-key.md) — the test side of "key is well-formed"
- [`EN-03-foreign-key-integrity.md`](./EN-03-foreign-key-integrity.md) — once types agree, content must too
