...
This rule assumes the Elementary setup from [`../model/volume-anomaly.md`](../model/volume-anomaly.md) (package, `+schema: elementary`, the dbt-1.8 `materialization_test_default` override). Elementary models are **prod-only** in this project, so this test is prod-scoped.
...
- **Tables with no natural load/event timestamp.** There's nothing to measure a gap against — fix the audit columns first ([`freshness-source-and-model.md`](./freshness-source-and-model.md)).

## See also

- [`freshness-source-and-model.md`](./freshness-source-and-model.md) — the fixed-SLA floor this escalates from (TM-AU-01)
- [`../model/volume-anomaly.md`](../model/volume-anomaly.md) — sibling Elementary anomaly test on row volume
- [`../model/column-anomalies.md`](../model/column-anomalies.md) — automated column-statistic monitors on the same engine
