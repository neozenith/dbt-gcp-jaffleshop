...
| Freshness degrading against its learned cadence (no fixed SLA fits) | system-time | [`freshness-anomalies.md`](./freshness-anomalies.md) |
...
1. **TM-SC-01** · [`event-time-bounds.md`](./event-time-bounds.md) — no future, no `1900-01-01` / `9999-12-31` sentinels
2. **TM-SC-02** · [`monotonic-pair.md`](./monotonic-pair.md) — `shipped_at >= ordered_at`, `updated_at >= created_at`
3. **TM-AU-01** · [`freshness-source-and-model.md`](./freshness-source-and-model.md) — source freshness + model recency
4. **TM-GR-01** · [`calendar-spine.md`](./calendar-spine.md) — `sequential_values` on `date_day`
5. **TM-AU-02** · [`scd2-quartet.md`](./scd2-quartet.md) — the four tests every Type-2 dim needs together
6. **TM-SC-03** · [`timezone-contract.md`](./timezone-contract.md) — TIMESTAMP vs DATETIME on BigQuery
7. **TM-AU-03** · [`freshness-anomalies.md`](./freshness-anomalies.md) — Elementary learned-band freshness / event-freshness anomalies
