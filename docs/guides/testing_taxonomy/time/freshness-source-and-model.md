...
- [`event-time-bounds.md`](./event-time-bounds.md) — for `event_at >= load_at` (causality)
- [`../model/volume-anomaly.md`](../model/volume-anomaly.md) — when freshness is fine but row count drops
- [`../dimension/dimension-anomalies.md`](../dimension/dimension-anomalies.md) — per-tenant freshness via dimension grouping
