...
| --- | --- | --- | --- | --- |
| EN-03 | entity | Consistency | Representational consistency | scan-bound |
...
If `fct_orders.customer_id` should only match `dim_customers` for non-guest orders, use [`EN-04-soft-delete-scoped-fk.md`](./EN-04-soft-delete-scoped-fk.md)'s `relationships_where`:
...
| `dbt_utils.relationships_where` | dbt-utils | Need `from_condition` / `to_condition` (soft-deletes, guest checkouts). See [`EN-04-soft-delete-scoped-fk.md`](./EN-04-soft-delete-scoped-fk.md). |
...
- [`EN-04-soft-delete-scoped-fk.md`](./EN-04-soft-delete-scoped-fk.md) — when the FK only applies under certain conditions
- [`EN-01-unique-key.md`](./EN-01-unique-key.md) — the parent-side test that makes the FK meaningful
- [`EN-06-type-stable-join.md`](./EN-06-type-stable-join.md) — type-mismatch is silent join failure
