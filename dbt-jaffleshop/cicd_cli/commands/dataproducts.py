...
# Local
from cicd_cli import config, style, viewer
...
class BoundaryTestRow:
    """One boundary node of a data product, with its test count and pass/fail."""
...
class SystemBoundaryReport:
    ...
    def summary(self) -> str:
        ...
        return f"all {len(self.rows)} boundary node(s) tested" if self.ok else f"{missing} boundary node(s) untested"
    ...
    def human_lines(self, *, show_passes: bool = False) -> list[tuple[int, str]]:
        ...
        if self.ok:
            lines: list[tuple[int, str]] = [
                (INFO, f"{label}  {style.passed(f'all {len(self.rows)} boundary node(s) have ≥1 test')}")
            ]
        else:
            missing = sum(1 for r in self.rows if not r.ok)
            lines = [(ERROR, f"{label}  {style.failed(f'{missing} boundary node(s) missing tests')}")]
        ...
        for product in dict.fromkeys(r.product for r in self.rows):  # stable, definition order
            ...
            for r in sorted(product_rows, key=_test_row_sort_key):
                ...
                if r.ok:
                    lines.append((INFO, style.pass_item(f"{desc} — {r.test_count} test(s)")))
                else:
                    lines.append((ERROR, style.fail_item(f"{desc} — no tests on this boundary node")))
        return lines
...
def evaluate_system_boundaries(graph: Graph, named_selectors: list[tuple[str, str]]) -> SystemBoundaryReport:
    """Classify each product's nodes and emit one row per BOUNDARY node with its test count."""
    data_nodes = set(graph.nodes())
    rows: list[BoundaryTestRow] = []
...
def cmd_check(args) -> int:
    graph, named_selectors = _select_named(args)
    return render_from_args(evaluate_system_boundaries(graph, named_selectors), args)
