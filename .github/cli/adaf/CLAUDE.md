...
>
> **dbt-engineering surface** — the dbt-workflow half, one capability per line:
> - **seam:** `run_dbt` / `ManifestView` / artifact (parse the manifest once; `[fusion]` parquet reader for dbt v2.0).
> - **`defer-diff` / `defer-state`:** built-vs-deferred split via a cached worktree-parsed baseline + deepdiff.
> - **`list` / `ls`:** scope preview — hop-walks, `--macros`, `--paths`; `--commands` dry-run on the gates.
> - **`gha`:** per-product workflow generator — `paths` derived from `dbt ls`, three glob algorithms + false-positive audit.
> - **`sdag check`:** boundary-obligation lint with catalogue rule IDs MD-02/11/12, TM-AU-01, MD-07.
> - **viewer:** selector cache (fingerprint-keyed, parallel resolution) + enrichment (compliance RAG rings, governance, inline/archive).
> - **`tests/multiversion/`:** Docker matrix across dbt 1.11/1.12/2.0, off `make ci`.
>
