...
# Local
from adaf.commands.dataproducts import (
    BoundaryReport,
    BoundaryTestRow,
    MemberRow,
    ProductBoundary,
...
def test_gate_fails_when_a_boundary_node_has_no_tests():
    ...
    assert report.summary() == "2 boundary node(s) under-protected"
...
def test_gate_passes_when_all_boundary_nodes_tested():
    ...
    assert report.ok is True
    assert "all 1 boundary node(s) satisfied" in _gate_text(report)
