...
// ─────────────────────────────────────────────────────────────────────────
// Design tokens (T35) — the brand aesthetic for the CANVAS lives in
...
let currentFilter = "__all__";  // "__all__" | <selector name from the dropdown> | UNMATCHED_SELECTOR
// Full-view display options (apply only to the Full graph view):
//   • hiddenTypes — resource_types to HIDE. `test` is hidden by default; the user can re-enable it
//     (and any other type) from the "Node types" toggles in the sidebar.
//   • showSiblings — show the 1-hop external upstream/downstream neighbours (the halo). OFF by
//     default so the view focuses on just the product's own members; tick to reveal the neighbours.
let hiddenTypes = new Set(["test"]);
let showSiblings = false;
// The synthetic "NO SELECTOR" bucket. Mirrors `UNMATCHED_ID` in viewer.py: its catch-all
// compound is `sel::__unmatched__`, but its member entities carry an EMPTY `selectors` list
// (they belong to no named selector), so the filter matches them by absence-of-membership,
// not by `selectors.includes(...)`.
const UNMATCHED_SELECTOR = "__unmatched__";
...
function applyStateFromUrl() {
  ...
  // Re-source the filter from the matched <option>'s own value rather than the
  // raw URL string: the option list is built from trusted graph data, so this
  // severs any taint carried by the user-controlled ?selector= param before it
  // is ever used as a compliance-map lookup key.
  const match = urlState.selector
    ? Array.from(select.options).find((o) => o.value === urlState.selector)
    : null;
  currentFilter = match ? match.value : "__all__";
...
function buildStyle(t) {
  return [
    ...
    // PARTIAL compliance (T-sdag) — a boundary node that meets SOME but not all of its obligations
    // gets a warn ring, not the binary red fail. The per-node compliance_pct drives which class.
    { selector: "node.compliance-partial", style: {
        "border-color": t.boundaryOutbound.border, "border-width": 4,
        "underlay-color": t.boundaryOutbound.bg, "underlay-opacity": 0.2, "underlay-padding": 6,
      } },
...
function renderCompliance(selector) {
  ...
  parts.push(
    `<div class="text-sub text-[11px]">` +
      ...
      ` · ${Number(c.passed) || 0} passed · ${Number(c.suppressed) || 0} suppressed` +
    `</div>`,
  );
  parts.push(
    `<div class="text-sub text-[11px] mt-0.5">` +
      `${Number(c.boundary_nodes) || 0} boundary nodes · <span class="${(c.failed_nodes ? "pct-fail" : "pct-ok")}">${Number(c.failed_nodes) || 0} failing</span>` +
    `</div>`,
  );
...
async function showView(name) {
  ...
  clearDetail();  // a fresh graph has nothing selected -> hide the selection panel
  cy = cytoscape({
...
// Build one node-type visibility checkbox per resource_type present in the full graph. `test` boots
// UNCHECKED (hidden by default); ticking a type re-shows it. Re-runs the filter on every change.
function populateTypeToggles(fullGraphData) {
  const wrap = document.getElementById("type-toggles");
  if (!wrap || wrap.childElementCount) return;
  const types = new Set();
  for (const e of fullGraphData.elements || []) {
    if (e.data && e.data.kind === "entity" && e.data.resource_type) types.add(e.data.resource_type);
  }
  for (const rt of Array.from(types).sort()) {
    const label = document.createElement("label");
    label.className = "ctl flex items-center gap-2 px-2 py-1 rounded text-xs cursor-pointer";
    const cb = document.createElement("input");
    cb.type = "checkbox";
    cb.checked = !hiddenTypes.has(rt);  // tests start hidden
    cb.addEventListener("change", () => {
      if (cb.checked) hiddenTypes.delete(rt); else hiddenTypes.add(rt);
      rerunWithFilter();
    });
    const swatch = document.createElement("span");
    swatch.style.cssText =
      `width:9px;height:9px;border-radius:2px;flex:none;background:${RESOURCE_COLOURS[rt] || "#94a3b8"};`;
    const txt = document.createElement("span");
    txt.textContent = rt;
    label.append(cb, swatch, txt);
...
// Only MODELS are boundary subjects (they own the obligations); the BACKBONE that boundary edges run
// over is the data nodes PLUS exposures (a model feeding an external exposure is outbound). Mirrors
// the server-side `adaf.dbt.graph.Graph`: subjects = models, edges include sources + exposures.
const isModelNode = (n) => n.data("resource_type") === "model";
const BOUNDARY_BACKBONE_TYPES = new Set(["model", "source", "seed", "snapshot", "exposure"]);
const isBackboneNode = (n) => BOUNDARY_BACKBONE_TYPES.has(n.data("resource_type"));

// Hide every entity whose resource_type is toggled off (full view only). A hidden node's edges go
// with it. Called after applyFilter() sets per-filter visibility, so type-hiding layers on top.
function applyTypeVisibility() {
  if (!cy || currentView !== "full") return;
  const hidden = cy.nodes(".entity").filter((n) => hiddenTypes.has(n.data("resource_type")));
...
function applyComplianceClasses() {
  ...
  for (const [uid, a] of Object.entries(ann)) {
    ...
    if (a.boundary === "inner") continue;  // inner nodes carry no obligations -> no compliance ring
    // PARTIAL credit: drive the ring off the per-node compliance_pct (fall back to a binary read for
    // older cache files). 100% = pass ring, 0% = fail ring + glow, anything between = a warn ring.
    const pct = typeof a.compliance_pct === "number"
      ? a.compliance_pct
      : (failingRuleIds(a).length ? 0 : 100);
    if (pct >= 100) node.addClass("compliance-pass");
    else if (pct <= 0) node.addClass("compliance-fail");
    else node.addClass("compliance-partial");
  }
}
...
function applyFilter() {
  ...
  if (currentView !== "full" || currentFilter === "__all__") {
    ...
    applyTypeVisibility();  // even with no selector filter, honour the node-type toggles
    return;
  }
  const targetCompoundId = `sel::${currentFilter}`;
  const compound = cy.getElementById(targetCompoundId);

  // The NO SELECTOR bucket has no named membership to match on — its entities carry an empty
  // `selectors` list — so select them by absence of membership; all other filters match by name.
  const members = currentFilter === UNMATCHED_SELECTOR
    ? cy.nodes(".entity").filter((n) => (n.data("selectors") || []).length === 0)
    : cy.nodes(".entity").filter((n) => (n.data("selectors") || []).includes(currentFilter));
  if (members.length === 0) {
  ...
  const hasExt = (n) => !isMember(n);  // external to the PRODUCT (for the 1-hop sibling halo)

  // ONLY MODELS are boundary subjects. The product INTERIOR is its model members, so a parent/child
  // that is a source, an exposure, or a model in another product is "external" and tips the model
  // inbound/outbound — mirroring the server-side Graph.classify(). Tests are not on the backbone.
  const modelMembers = members.filter(isModelNode);
  const modelMemberIds = new Set(modelMembers.map((n) => n.id()));
  const isInterior = (n) => modelMemberIds.has(n.id());  // external-to-interior makes a model inbound/outbound

  const inboundBoundary = modelMembers.filter((m) => {
    const parents = m.incomers("node").filter(isBackboneNode);
    return parents.filter((p) => !isInterior(p)).length > 0 || parents.filter(isInterior).length === 0;
  });
  const outboundBoundary = modelMembers.filter((m) => {
    const children = m.outgoers("node").filter(isBackboneNode);
    return children.filter((c) => !isInterior(c)).length > 0 || children.filter(isInterior).length === 0;
  });
  ...
  // 1-hop siblings = the boundary models' backbone neighbours that are OUTSIDE the product entirely.
  const upstreamExt = inboundBoundary.incomers("node").filter(isBackboneNode).filter(hasExt);
  const downstreamExt = outboundBoundary.outgoers("node").filter(isBackboneNode).filter(hasExt);
  ...
  // The product itself is always visible; the 1-hop sibling halo only when `showSiblings` is on.
  let visible = compound.union(members).union(internalEdges);
  if (showSiblings) {
    visible = visible
      .union(upstreamExt).union(downstreamExt).union(haloCompounds)
      .union(upstreamEdges).union(downstreamEdges);
  }
  ...
  applyTypeVisibility();      // hide toggled-off node types (e.g. tests by default)
  applyComplianceClasses();   // compliance rings layer on top of the boundary colours
}
...
// Show/hide the whole selected-node panel (it sits above the compliance panel and is hidden until
// something is selected).
function setSelectionVisible(on) {
  const panel = document.getElementById("detail");
  if (panel) panel.classList.toggle("visible", !!on);
}

// The node-type / boundary ATTRIBUTION for the selected node under the active selector: the specific
// reasons it was labelled inbound/outbound/both/inner (the external refs that crossed the boundary).
// Pulled straight from the cache annotation embedded in full_graph.json.
function boundaryAttributionHtml(uid) {
  const entry = complianceFor(currentFilter);
  if (!entry) return "";
  const a = (entry.annotations || {})[uid];
  if (!a || !Array.isArray(a.attribution) || !a.attribution.length) return "";
  const axisCls = (axis) =>
    axis === "inbound" ? "badge-pass" : axis === "outbound" ? "badge-warn" : "badge-soft";
  const rows = a.attribution.map((r) => {
    const nodes = (r.nodes && r.nodes.length)
      ? `<div class="text-sub text-[10px] mono break-all pl-6">` +
        r.nodes.map((n) => escapeHtml(n)).join("<br/>") + `</div>`
      : "";
    return `<div class="gov-row py-0.5">` +
      `<span class="badge ${axisCls(r.axis)}">${escapeHtml(r.axis)}</span>` +
      `<span class="text-body text-[11px]">${escapeHtml(r.message)}</span></div>` + nodes;
  }).join("");
  return [
    `<div class="divider mt-2 pt-2">`,
    `<div class="text-label uppercase tracking-wider text-[10px] mb-1">Why ${escapeHtml(a.boundary)} · attribution</div>`,
    rows,
    `</div>`,
  ].join("");
}
...
function renderDetail(data) {
  ...
  body.innerHTML =
    head + governanceHtml(data) + boundaryAttributionHtml(data.id) + complianceDetailHtml(data.id);
}
...
function buildBoundaryReport(superName) {
  ...
  // NO SELECTOR's members carry an empty `selectors` list (they belong to no named selector),
  // so match them by absence of membership; every other super matches by name.
  const isUnmatched = superName === UNMATCHED_SELECTOR;
  const members = new Set();
  for (const e of fullData.elements) {
    if (e.data.kind !== "entity") continue;
    const sel = e.data.selectors || [];
    if (isUnmatched ? sel.length === 0 : sel.includes(superName)) {
...
function renderBoundaryReport(report) {
  const body = document.getElementById("detail-body");
  if (!report) {
    setSelectionVisible(false);  // nothing meaningful to show -> hide the panel
    return;
  }
  setSelectionVisible(true);
...
function clearDetail() {
  setSelectionVisible(false);  // hide the whole panel when nothing is selected
  const body = document.getElementById("detail-body");
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  document.getElementById("opt-siblings").addEventListener("change", (e) => {
  ...
  document.getElementById("filter-selector").addEventListener("change", (e) => {
    ...
    setFilterLegendVisible(currentFilter !== "__all__");
    clearDetail();  // the selected node may not belong to the newly-chosen product -> reset selection
  ...
  try {
    ...
    populateTypeToggles(cache.full);
