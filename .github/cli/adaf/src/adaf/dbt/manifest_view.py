...
coverage view (``adaf.dbt.manifest.Manifest``), the lineage graph
(``adaf.dbt.graph.Graph``), the boundary-lint artifact sets
(``adaf.commands.sdaglint.Artifacts``), and the sdag viewer's display graph
(``adaf.sdag.viewer.load_full_manifest``). Each used to re-do the same mechanical work — read the
file, iterate the ``nodes`` + ``sources`` sections (defaulting a source's ``resource_type``),
flatten ``parent_map`` into edges, and find the ``test`` nodes. That duplication lived in four
places; a dbt schema change meant four edits.
...
via a ``from_view`` constructor (the projections' existing ``load`` / ``from_dict`` /
``from_manifest`` entry points remain as thin wrappers, so call sites and tests are unchanged).
