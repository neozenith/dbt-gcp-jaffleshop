...
one with module:func:line context (matches the sibling ``tfs`` tool). Human and
log lines go to stderr; the ``--json`` payload is the only thing on stdout, so
``adaf ... --json | jq`` stays clean."""
