...
# Column specs: (header, row-key). Reused by both renderers; the priced view drops a few.
FULL_COLS = [
    ("Model", "model"), ("Provider", "provider"), ("Tier", "tier"), ("Free rpm/rpd", "rate"),
    ("Ctx in", "ctx"), ("Input modes", "modes"), ("$/1M in", "in_p"), ("$/1M out", "out_p"),
    ("Bench resp_format", "rf"), ("Bench out tok", "out"), ("Bench findings", "findings"),
    ("Bench req (s)", "req_s"), ("Wall (s)", "wall"), ("Back-off %", "backoff"),
    ("Bench tok/s", "tok_s"), ("Bench cost", "cost"), ("Bench status", "status"),
]
PRICED_COLS = [c for c in FULL_COLS if c[0] not in {"Ctx in", "Input modes"}]
...
def main() -> None:
    ...
    caption = (f"Canonical table — {len(rows)} catalogue models LEFT-JOINed with pricing "
               ...
               f"Blank/— = not priced / not benchmarked. **Free rpm/rpd** = free-tier rate limit by "
               f"tier; **Wall (s)** = `Bench req (s)` + rate-limit back-off; **Back-off %** = "
    ...
    note = (
        ...
        f"(Findings desc, then Bench cost asc). **Free rpm/rpd** = free-tier rate limit by tier; "
        f"**Wall (s)** = `Bench req (s)` + rate-limit back-off; **Back-off %** = back-off ÷ wall. "
        f"Generated by `join.py` (do not hand-edit).\n")
    if excluded:
